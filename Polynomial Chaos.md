...
by summing and scaling its elements, we can generate every other vector in the space (in addition, the element themselves are independent of each other,
ie none of them can be generated by the others).

```math
\mathbf{v} = v_0 \mathbf{e_0} + v_1 \mathbf{e_1} + \cdots
```

Equipped with this definition, we notice how the rappresentation of a vector as array or tuple of numbers is simply an [isomorphism](https://en.wikipedia.org/wiki/Isomorphism) 
